...
{
    Acc::VideoInfo accVideoInfo{};
    auto ret = Acc::GetVideoInfo(path, accVideoInfo);
    if (ret != 0)
    {
        throw std::runtime_error("Failed to get video info, please see above log for detail.");
    }

    VideoInfo result{};
    result.fps = accVideoInfo.fps;
    result.nFrames = accVideoInfo.nFrames;
    result.durationSec = accVideoInfo.durationSec;
    return result;
}
