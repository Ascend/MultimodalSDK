...
const std::map<Acc::DataType, std::string> DATA_TYPE_TO_FORMAT = {
    {Acc::DataType::INT8, "|i1"}, {Acc::DataType::UINT8, "|u1"}, {Acc::DataType::FLOAT32, "<f4"}  // Little-endian
};
}  // namespace
namespace PyAcc
{
// RAII守卫：确保函数体内操作Python对象时持有GIL。
// 开启swig -threads后，wrapper进入C++函数前会释放GIL，
// 若C++函数体内直接调用Python C API（如numpy互转），必须在无GIL状态下重新获取。
// PyGILState_Ensure可重入，在已持有GIL的场景下调用也是安全的。
class PyGilStateGuard
{
   public:
    PyGilStateGuard() : gilState_(PyGILState_Ensure()) {}
    ~PyGilStateGuard() { PyGILState_Release(gilState_); }
    PyGilStateGuard(const PyGilStateGuard&) = delete;
    PyGilStateGuard& operator=(const PyGilStateGuard&) = delete;
    PyGilStateGuard(PyGilStateGuard&&) = delete;
    PyGilStateGuard& operator=(PyGilStateGuard&&) = delete;
...
{
    // 本函数全程操作Python对象，必须持有GIL
    PyGilStateGuard gilGuard;
    if (!pyObj)
    {
    ...
    if (!arrayInterface)
    {
        throw std::runtime_error(
            "The python numpy ndarray does not have the __array_interface__ dictionary in "
    ...
    std::shared_ptr<PyObject> arrayInterfacePtr(arrayInterface,
                                                [](PyObject* obj)
                                                {
                                                    if (obj != nullptr)
                                                    {
    ...
    {
        throw std::runtime_error(
            "Invalid data field in __array_interface__ of python numpy ndarray, "
    ...
    {
        throw std::runtime_error(
            "Failed to get valid data pointer from __array_interface__ of python numpy "
    ...
    {
        throw std::runtime_error(
            "Invalid shape field in __array_interface__ of python numpy ndarray. "
    ...
    for (Py_ssize_t i = 0; i < PyTuple_Size(shapeTuple); i++)
    {
        ...
        {
            throw std::runtime_error(
                "Invalid dimension in shape of __array_interface__ of python numpy "
    ...
    {
        throw std::runtime_error(
            "Invalid typestr field in __array_interface__ of python numpy ndarray. "
    ...
    {
        throw std::runtime_error(
            "Encode the typestr field's value in __array_interface__ to string failed. "
    ...
    {
        throw std::runtime_error(
            "Unsupported python numpy ndarray datatype. "
...
PyObject* ToNumpy(NumpyData numpyData)
{
    ...
    {
        throw std::runtime_error(
            "Unexpected error, failed to create interface dictionary for __array_interface__ "
    ...
    if (!obj)
    {
        Py_DECREF(interface);
        throw std::runtime_error(
            "Failed to set __array_interface__ attribute on return object."
            " Python interpreter may be out of memory.");
    }
