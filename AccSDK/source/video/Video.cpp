                                  ...
                                  const std::vector<int>& targetKeyframeIndices, std::map<int, AVFrame*>& results)
{
    ...
    for (size_t i = 0; i < targetKeyframeIndices.size(); i++)
    {
        ...
        futures.push_back(pool.Submit(
            [i, &targetKeyframeIndices, &videoAuxInfo, path, &results, &errorOccurred, &resultsMutex]()
            {
                               ...
                               std::vector<Image>& frames)
{
    ...
    for (AVFrame* rgbFrame : rgbFrames)
    {
        ...
        std::shared_ptr<uint8_t> framePtr(rgbFrame->data[0],
                                          [](uint8_t* ptr)
                                          {
                                              if (ptr)
                                              {
...
ErrorCode GetVideoInfo(const char* path, VideoInfo& info)
{
    if (path == nullptr)
    {
        LogError << "Get video info failed, path is nullptr." << GetErrorInfo(ERR_INVALID_POINTER);
        return ERR_INVALID_POINTER;
    }
    if (!IsFileValid(path))
    {
        LogError << "Get video info failed, video path is invalid." << GetErrorInfo(ERR_INVALID_PARAM);
        return ERR_INVALID_PARAM;
    }
    if (!CheckFileExtension(path, "mp4"))
    {
        LogError << "Get video info failed, invalid video suffix, only support 'mp4', 'MP4'."
                 << GetErrorInfo(ERR_INVALID_PARAM);
        return ERR_INVALID_PARAM;
    }
    ...
    double originFps = 0;
    int64_t totalFrames = 0;
    ErrorCode ret = GetFramesAndFPS(videoStream, originFps, totalFrames);
    if (ret != SUCCESS)
    {
        avformat_close_input(&formatCtx);
        LogError << "Cannot determine video frame count or FPS, may caused by input video which is broken."
    ...
    // duration: stream duration first (matches the PyAV baseline: stream.duration * time_base),
    // container duration as fallback (formatCtx->duration / AV_TIME_BASE)
    double durationSec = 0;
    if (videoStream->duration != AV_NOPTS_VALUE)
    {
        durationSec = videoStream->duration * av_q2d(videoStream->time_base);
    }
    // guard against NaN/Inf from a broken stream time_base, then fall back to container duration
    if (!std::isfinite(durationSec) || durationSec <= 0)
    {
        durationSec = 0;
    }
    if (durationSec <= 0 && formatCtx->duration != AV_NOPTS_VALUE)
    {
        durationSec = static_cast<double>(formatCtx->duration) / static_cast<double>(AV_TIME_BASE);
    }
    avformat_close_input(&formatCtx);

    info.fps = originFps;
    info.nFrames = totalFrames;
