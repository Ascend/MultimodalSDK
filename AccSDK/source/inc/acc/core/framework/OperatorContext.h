...
struct OperatorContext
{
    ...
    /**
     * @brief Copy constructor: performs member-wise copy of input/output tensor references
     */
    OperatorContext(const OperatorContext&) = default;
    /**
     * @brief Copy assignment operator: assigns from another OperatorContext
     */
    OperatorContext& operator=(const OperatorContext&) = default;
    /**
     * @brief Move constructor: enables efficient transfer of tensor references
     */
    OperatorContext(OperatorContext&&) = default;
    /**
     * @brief Move assignment operator: enables efficient transfer from another context
     */
    OperatorContext& operator=(OperatorContext&&) = default;
...
struct ResizeContext : OperatorContext
{
    ...
    ResizeContext(const std::vector<std::reference_wrapper<const Tensor>>& inputTensorRefs,
                  const std::vector<std::reference_wrapper<Tensor>>& outputTensorRefs, size_t resizedH, size_t resizedW,
                  const Interpolation interpolation, DeviceMode deviceMode)
...
struct NormalizeContext : OperatorContext
{
    ...
    NormalizeContext(const std::vector<std::reference_wrapper<const Tensor>>& inputTensorRefs,
                     const std::vector<std::reference_wrapper<Tensor>>& outputTensorRefs, const std::vector<float> mean,
                     const std::vector<float> stddev, DeviceMode deviceMode)
...
struct QwenFusionContext : OperatorContext
{
    ...
    QwenFusionContext(const std::vector<std::reference_wrapper<const Tensor>>& inputTensorRefs,
                      const std::vector<std::reference_wrapper<Tensor>>& outputTensorRefs,
                      const std::vector<float>& mean, const std::vector<float>& std, int resizeH, int resizeW,
                      TensorFormat layout, DeviceMode deviceMode)
        : OperatorContext(inputTensorRefs, outputTensorRefs),
          mean(mean),
          std(std),
          resizeH(resizeH),
...
struct ToTensorContext : OperatorContext
{
    ...
    ToTensorContext(const std::vector<std::reference_wrapper<const Tensor>>& inputTensorRefs,
                    const std::vector<std::reference_wrapper<Tensor>>& outputTensorRefs, TensorFormat format,
                    DeviceMode deviceMode)
