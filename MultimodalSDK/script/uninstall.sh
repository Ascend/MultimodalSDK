...
get_run_path() {
  run_path=$(pwd)
  cd ..
  if [[ "$run_path" =~ /multimodal ]];then
    suffix='multimodal'
  else
    log "Error: Directory multimodal does not exist, exiting!"
    echo "Directory multimodal does not exist in path[$run_path], exiting!"
    exit 1
  fi
  del_path=$(pwd)/"$suffix"
}
real_delete() {
  cd "${CUR_PATH}/.." || {
    echo "Cannot locate Multimodal SDK directory: ${CUR_PATH}/.."
    log "Error: Cannot locate Multimodal SDK directory!"
    exit 255
  }
  get_run_path
  if [[ -f "$del_path"/version.info ]];then
    ...
    log "Uninstall Multimodal SDK package successfully."
    echo 'Uninstall Multimodal SDK package successfully.'
  else
    log "Uninstall Multimodal SDK package failed, version info is missed."
    echo 'Uninstall Multimodal SDK package failed, version info is missed.'
  fi
}
