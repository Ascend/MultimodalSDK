...
5. **开发构建验证**：
   ...
   环境准备请参阅[安装指南](docs/zh/03_installation_guide/installation_guide.md)。
   下载代码后直接执行 `bash build_script/build_merge.sh`，脚本会自动下载并准备全部编译依赖：
...
6. **本地门禁检查**：请先运行 [pre-commit](https://gitcode.com/Ascend/community/blob/master/docs/contributor/pre-commit-guide.md) 进行本地检查，确保代码符合规范。
