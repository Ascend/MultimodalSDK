...
Multimodal SDK 支持[离线安装](#离线安装)（`run` 包 / `Wheel` 包）、[镜像安装](#镜像安装)、[源码安装](#源码安装)四种方式。
...
请从[获取链接](https://gitcode.com/Ascend/MultimodalSDK/releases)下载 Multimodal SDK 特征检索软件包（Ascend-mindxsdk-multimodal_{version}_linux-{arch}.run）。
