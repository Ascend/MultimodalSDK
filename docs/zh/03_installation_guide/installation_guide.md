...
本文档适用于 Multimodal SDK 最新版本的安装，仅支持硬件为Atlas 800I A2推理服务器，操作系统为Ubuntu 22.04或openEuler 24.03，建议预留不少于16 GB可用磁盘空间和不少于8 GB可用内存。

Multimodal SDK 支持[离线安装](#离线安装)（`run` 包 / `Wheel` 包）、[源码安装](#源码安装)、[镜像安装](#镜像安装)三种方式。
...
请参考[CANN安装指南](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/900/softwareinst/instg/instg_0000.html)，使用CANN（Compute Architecture for Neural Networks）9.0.0及对应驱动版本完成NPU（Neural Processing Unit）驱动固件和CANN的安装。HDK 指 Ascend HDK（Hardware Development Kit），需与 CANN 和 Multimodal SDK 版本配套。
...
| Python       | 最低 3.9；**推荐 3.10 或 3.11** | 建议通过包管理器安装：<br>Ubuntu：`sudo apt-get install -y python3 python3-pip python3-dev`<br>openEuler：`sudo yum install -y python3 python3-pip python3-devel`<br>若系统自带版本过低，可从源码编译或安装更高版本 |
...
| torch        | 2.5.1                     | 通过 pip 安装：<br>`pip3 install torch==2.5.1`<br>ARM64 昇腾环境需额外配置 |
| torch-npu    | 2.5.1.post1               | 通过 pip 安装：<br>`pip3 install torch-npu==2.5.1.post1`<br>需与 CANN 9.0.0 配套 |

## 安装方式

Multimodal SDK 提供四种安装方式：离线安装（`run` 包 / `Wheel` 包）、源码安装和镜像安装，可根据场景选择合适的方式。
...
请从[获取链接](https://gitcode.com/Ascend/MultimodalSDK/releases)下载 Multimodal SDK 特征检索软件包（Ascend-mindxsdk-multimodal_{version}_linux-{arch}.run）。其中 `{version}` 为 SDK 版本号（例如 `26.0.0`），`{arch}` 为 CPU 架构（`x86_64` 或 `aarch64`）。
...
> - 主机上需安装与容器内 CANN 版本兼容的 NPU 驱动（详见 [安装NPU驱动固件和CANN](#安装npu驱动固件和cann)）。
...
| 更多问题                      | [FAQ - 安装与环境](../06_references/faq.md#安装与环境)、[附录 - 错误码](../06_references/appendix.md#错误码) |
