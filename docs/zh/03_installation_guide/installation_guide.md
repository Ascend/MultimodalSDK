...
请参考 [CANN 安装指南](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/900/softwareinst/instg/instg_0000.html)，使用 CANN（Compute Architecture for Neural Networks）9.0.0 及 HDK（Hardware Development Kit）26.0.RC1 完成 NPU（Neural Processing Unit）驱动固件和 CANN 的安装。
...
请从 [Multimodal SDK Releases](https://gitcode.com/Ascend/MultimodalSDK/releases) 下载 Multimodal SDK 软件包（Ascend-mindxsdk-multimodal_{version}_linux-{arch}.run）。其中 `{version}` 为 SDK 版本号（例如 `26.0.0`），`{arch}` 为 CPU 架构（`x86_64` 或 `aarch64`）。
