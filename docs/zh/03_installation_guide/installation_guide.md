...
请参考[CANN安装指南](https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/900/softwareinst/instg/instg_0000.html)，使用CANN（Compute Architecture for Neural Networks）9.1.0及对应驱动版本完成NPU（Neural Processing Unit）驱动固件和CANN的安装。HDK 指 Ascend HDK（Hardware Development Kit），需与 CANN 和 Multimodal SDK 版本配套。
...
| torch-npu    | 2.5.1.post1               | 通过 pip 安装：<br>`pip3 install torch-npu==2.5.1.post1`<br>需与 CANN 9.1.0 配套 |
...
请从[获取链接](https://gitcode.com/Ascend/MultimodalSDK/releases)下载 Multimodal SDK 特征检索软件包（Ascend-mindxsdk-multimodal_{version}_linux-{arch}.run）。其中 `{version}` 为 SDK 版本号（例如 `26.1.0`），`{arch}` 为 CPU 架构（`x86_64` 或 `aarch64`）。
