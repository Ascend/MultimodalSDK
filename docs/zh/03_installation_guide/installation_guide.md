...
本文档适用于 Multimodal SDK 最新版本的安装，仅支持硬件为 Atlas 800I A2 推理服务器，操作系统为 Ubuntu 22.04 或 openEuler 24.03，建议预留不少于 16GB 可用磁盘空间和不少于 8GB 可用内存。Multimodal SDK 当前为正式支持版本，搭配 9.0.0 版本的 CANN 和相关配套。
...
请从[Multimodal SDK Releases](https://gitcode.com/Ascend/MultimodalSDK/releases)下载Multimodal SDK 软件包（Ascend-mindxsdk-multimodal_{version}_linux-{arch}.run）。其中 `{version}` 为 SDK 版本号（例如 `26.1.0`），`{arch}` 为 CPU 架构（`x86_64` 或 `aarch64`）。
